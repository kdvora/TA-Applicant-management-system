...
const FeedbackList = ({ setUser }) => {
	...
	useEffect(() => {
		const fetchAllFeedbacks = async () => {
			try {
				const response = await axios.get("http://localhost:4000/api/getAllFeedbacks");
								...
								<Card style={{ backgroundColor: "#1DA1F2", width: "80%", margin: '0px auto', borderRadius: "35px" }}>
									{feedbacks.map((feedback, index) => (
										<Card
											...
											style={{ backgroundColor: "rgb(255,255,255)", width: "80%", margin: '0px auto', borderRadius: "25px" }}
										>
